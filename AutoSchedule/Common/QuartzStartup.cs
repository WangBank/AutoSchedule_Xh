...
namespace AutoSchedule.Common
{
    public class QuartzStartup
    {
           ...
           string messageName = e.message.msgName;
            try
            {
                switch (messageName)
                {
                    ...
                    case "stockChangeFeedbackMsg":
                        StockChangeFeedbackMsg(e);
                        break;
                    default:
        ...
        /// </summary>
        /// <param name="e"></param>
        private void StockChangeFeedbackMsg(JdEventArgs e)
        {
            _logger.LogInformation("{EventId}:\r\n{result}", "库存异动回传", JsonConvert.SerializeObject(e));
           
            var stock = JsonConvert.DeserializeObject<StockChangeFeedbackMsg>(e.message.msgPayload);

            List<STOCKCHANGE_XH_JDWMS> stockinfeedbackdetailsList = new List<STOCKCHANGE_XH_JDWMS>();
            for (int i = 0; i < stock.items.Count; i++)
            {
                string billguid = Guid.NewGuid().ToString("N");
                 var changelist= new STOCKCHANGE_XH_JDWMS
                {
                    CHANGETIME = stock.items[i].changeTime,
                    BATCHCODE = stock.items[i].batchCode,
                    ERPSTATUE = "0",
                    INVENTORYTYPE = stock.items[i].inventoryType,
                    ISLOCKED = stock.items[i].isLocked,
                    ITEMCODE = stock.items[i].itemCode,
                    ORDERCODE = stock.items[i].orderCode,
                    GUID = billguid,
                    WAREHOUSECODE = stock.items[i].warehouseCode,
                    ORDERTYPE = stock.items[i].orderType,
                    OUTBIZCODE = stock.items[i].outBizCode,
                    OWNERCODE = stock.items[i].ownerCode,
                    PRODUCECODE = stock.items[i].produceCode,
                    QUANTITY = stock.items[i].quantity,
                    ITEMID = stock.items[i].itemId,
                    REMARK = stock.items[i].remark
                };
                var insertDetailCount = _fsql.Insert(changelist).ExecuteAffrows();
                if (insertDetailCount == 1)
                {
                    //调用存储过程WMS_JDWMS_STOCKCHANGE
                    ...
                    _fsql.Ado.ExecuteNonQuery(CommandType.StoredProcedure, "WMS_JDWMS_STOCKCHANGE", BILLGUID, RETURNMSG, RETURNVALUE);
                    if (RETURNVALUE.Value.ToString() != "0")
                    {
                        _logger.LogInformation("{EventId}:\r\n{result}", "库存异动回传保存数据,执行存储过程错误", RETURNMSG.Value);
                    }
                    else
                    {
                        _logger.LogInformation("{EventId}:\r\n{result}", "库存异动回传保存数据,执行存储过程WMS_JDWMS_STOCKCHANGE,返回:", RETURNMSG.Value);
                    }
                }
                else
                {
                    _logger.LogInformation("{EventId}:\r\n{result}", "库存异动回传保存数据错误:", JsonConvert.SerializeObject(e));
                }
            }
        }
